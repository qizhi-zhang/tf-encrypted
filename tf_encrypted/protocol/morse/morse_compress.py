...
from compress import  compress_int64 as compress, de_compress_int64 as de_compress
...
class Morse(SecureNN):
    ...
    def assistant_OT_nocompress(self, y: AbstractTensor, j: AbstractTensor)->  AbstractTensor:
    ...
    def assistant_OT_lrcompress(self, y: AbstractTensor, j: AbstractTensor)->  AbstractTensor:
        #innput: y [n1,n2,...,nk-1, nk ] in device0,  j [n1, n2, ..., nk-1] in device1 modulus=nk
        # output: yj in device1 [n1, n2, ..., nk-1] dtype(yj)==dtype(y)
        ...
        with tf.device(self.server_0.device_name):
            y_compressed=compress(y)

        with tf.device(self.server_0.device_name):
            j_compressed=compress(j)

        with tf.device(self.server_2.device_name):
            y=de_compress(*y_compressed)
            j=de_compress(*j_compressed)
            ...
            i_compressed=compress(i)
            x_reshape_compressed=compress(x_reshape)
        with tf.device(self.server_1.device_name):
            i=de_compress(*i_compressed)
            j=de_compress(*j_compressed)

            k=i-j
            k_reshape = k.reshape(axes=[-1])

            k_reshape_compressed=compress(k_reshape)
        with tf.device(self.server_0.device_name):
            x_reshape=de_compress(*x_reshape_compressed)
            k_reshape=de_compress(*k_reshape_compressed)
            ...
            z_reshape_compressed=compress(z_reshape)
        with tf.device(self.server_1.device_name):
            z_reshape=de_compress(*z_reshape_compressed)
    ...
    def assistant_OT_rcompress(self, y: AbstractTensor, j: AbstractTensor)->  AbstractTensor:
        #innput: y [n1,n2,...,nk-1, nk ] in device0,  j [n1, n2, ..., nk-1] in device1 modulus=nk
        # output: yj in device1 [n1, n2, ..., nk-1] dtype(yj)==dtype(y)
    ...
    def left_equal_right_bits(self, x_bits: AbstractTensor, y_bits: AbstractTensor,
                              output_modulus=None) -> PondPrivateTensor:
        """
        ...
        print("output_modulus@left_equal_right_bits=", output_modulus)
    ...
    def left_leq_right_bits(self, x: AbstractTensor, y: AbstractTensor, output_modulus=None) -> PondPrivateTensor:
        """
        \
    ...
    def left_leq_right(self, x: AbstractTensor, y: AbstractTensor, output_modulus=None) -> PondPrivateTensor:
        """
