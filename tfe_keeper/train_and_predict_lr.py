...
from tfe_keeper.read_data_tf import get_data_xy, get_data_x, get_data_y, get_data_id_with_y, get_data_id_with_xy
...
def run(taskId, conf, modelFileMachine, modelFilePath, modelFilePlainTextPath, tf_config_file=None, clip_value=5.0):
    ...
    try:
        ...
        if (featureNumY == 0):
            ...
            x_train = prot.define_local_computation(player='XOwner', computation_fn=get_data_x, 
                                                    arguments=(batch_size, path_x, featureNumX, 
                                                               matchColNumX, epoch_num * 2, clip_value, 1))
            ...
            x_train1, y_train = prot.define_local_computation(player='YOwner', computation_fn=get_data_xy, 
                                                              arguments=(batch_size, path_y, featureNumY, 
                                                                         matchColNumY, epoch_num * 2, clip_value, 1))
        ...
        def provide_test_data_x(path):
            x = get_data_x(batch_size, path, featureNum=featureNumX, matchColNum=matchColNumX, epoch=2,
                           clip_by_value=clip_value, skip_row_num=1)
            return x
...
if __name__ == '__main__':
    ...
    with open('./qqq/conf_ym5w', 'r') as f:
        conf = f.read()
        print(conf)
