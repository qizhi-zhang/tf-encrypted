...
from read_data_tf import get_data_xy, get_data_x, get_data_y, get_data_id_with_x
...
def run(taskId,algorithm,conf,modelFileMachine,modelFilePath):
    ...
    def provide_test_data_x(path="/Users/qizhi.zqz/projects/TFE/tf-encrypted/examples/test_on_morse_datas/data/embed_op_fea_5w_format_x.csv"):
        idx, x = get_data_id_with_x(batch_size, path, featureNum=featureNumX, matchColNum=matchColNumX, epoch=2, clip_by_value=3.0, skip_row_num=1)
        return idx,x
    ...
    def provide_test_data_y(path="/Users/qizhi.zqz/projects/TFE/tf-encrypted/examples/test_on_morse_datas/data/embed_op_fea_5w_format_y.csv"):
        y = get_data_y(batch_size, path, matchColNum=matchColNumX, epoch=2,  skip_row_num=1)
        return y
    ...
    def provide_test_data_xy(path="/Users/qizhi.zqz/projects/TFE/tf-encrypted/examples/test_on_morse_datas/data/embed_op_fea_5w_format_y.csv"):
        x, y = get_data_xy(batch_size, path, featureNum=featureNumY, matchColNum=matchColNumX, epoch=2, clip_by_value=3.0, skip_row_num=1)
        return x, y

    if (featureNumY==0):
        idx, x_test = provide_test_data_x(path_x)
        y_test = provide_test_data_y(path_y)
    else:
        x_test1, y_test=provide_test_data_xy(path_y)
        idx, x_test0=provide_test_data_x(path_x)
    ...
    with tfe.Session() as sess:
        ...
        model.predict(sess, x_test, "./qqq/predict", batch_num, idx)
